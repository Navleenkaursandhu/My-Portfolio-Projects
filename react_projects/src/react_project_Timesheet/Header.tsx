  ...
  return (
    <>
      <div className='flex px-[30px] pl-[10px] m-[10px] justify-between items-center bg-[#2f3a40] h-[70px]'>
        <button onClick={() => toggleButton()} className='shadow hover:shadow-lg hover:shadow-gray-400 text-[28px] text-white bg-[#febe07] px-[10px]'>{userEventType}</button>
        ...
        {userIsSignedIn && <button onClick={() => addTaskInSheet()} className="shadow hover:shadow-lg hover:shadow-gray-400 text-[30px] bg-[#febe07] text-white px-[10px]">ADD TASK</button>}
        <div className='flex items-center justify-between w-[30%]'>
          <div onClick={() => dayBefore()} className='hover:cursor-pointer hover:grayscale border-y-[18px]  border-r-[18px] border-r-[#febe07] border-y-transparent'></div>
          <div className="text-[30px] bg-[#febe07] text-white px-[10px] py-[2px] flex items-center">Date: {format(date, 'do MMMM yyyy')}</div>
          <div onClick={() => dayAfter()} className='hover:cursor-pointer hover:grayscale border-y-[18px]  border-l-[18px] border-l-[#febe07] border-y-transparent'></div>
        </div>
      </div>
